...
bg-[#1a525f] 
border border-[rgba(22,180,198,0.25)] 
shadow-xl shadow-black/40 
p-8">
        <h2 className="text-2xl text-green-400 font-bold text-center mb-8">Profile</h2>
        ...
        <div className="mb-5">
          <label className="text-sm text-pink-600">Role</label>
          <div className="flex items-center border-b border-gray-300 py-2">
            <span className="text-pink-400 mr-2">🎓</span>
            <span className="text-sm text-green-400">{profile?.role}</span>
          </div>
        </div>
        ...
        {profile.department && (
          <div className="mb-5">
            <label className="text-sm text-pink-600">Department</label>
            <div className="flex items-center border-b border-gray-300 py-2">
              <span className="text-green-400 mr-2">🏢</span>
              <span className="text-sm text-green-400">{profile?.department}</span>
            </div>
          </div>
        )}
        {!isUpdate &&
          <button
            ...
            className="w-full mt-6 bg-amber-400 text-black py-2 rounded hover:bg-amber-700 transition hover:rounded-lg cursor-pointer hover:scale-90"
          >
            Update Profile
          </button>
        }
        {isUpdate &&
          <button
            ...
            className="w-full mt-6 bg-green-400 text-black py-2 rounded hover:bg-green-600 transition hover:rounded-lg cursor-pointer hover:scale-90"
          >
            Save
          </button>
        }
...
function Field({ icon, label, value, onChange, name, editable }) {
  return (
    <div className="mb-5">
      <label className="text-sm text-pink-600">{label}</label>
      <div className="flex items-center border-b border-gray-300 py-2">
        ...
        <input
          ...
          onChange={onChange}
          disabled={!editable}
          className={`w-full outline-none text-sm text-green-400 bg-transparent ${!editable ? "text-gray-500" : "text-black"
            }`}
        />
      </div>
    </div>
